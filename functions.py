...
import sys
import PIL
from scipy.signal.windows.windows import slepian
import main
...
def find_diff(hwnd):
    ...
    diff_2_3 = (diff_2_3 * 255).astype("uint8")
    ...
    thresh_2_3 = cv2.threshold(
        diff_2_3, 0, 255, cv2.THRESH_BINARY_INV | cv2.THRESH_OTSU)[1]

    contours = cv2.findContours(
        thresh.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)

    contours_2_3 = cv2.findContours(
        thresh_2_3.copy(), cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
    ...
    for c in contours:
        ...
        if area > 350:
            ...
            if (x in range(250,285,1) or x in range(230, 300)) and (y in range(490,550,1) or y in range(930, 1000, 1)):
                cv2.rectangle(
    ...
    # cv2.imwrite('images/after.jpg', after)
    # cv2.imwrite('images/diff.jpg', diff)
    return mobs_pos
    # cv2.imwrite('images/mask.jpg', mask)
    # cv2.imwrite('images/filled after.jpg', filled_after)
